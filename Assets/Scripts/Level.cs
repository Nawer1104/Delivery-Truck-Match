...
public class Level : MonoBehaviour
{
    ...
    private bool AreObjectsMatching(List<GameObject> objects)
    {
        ...
        Dictionary<Enum, int> typeCounts = new Dictionary<Enum, int>();

        // Count the occurrences of each object type
        foreach (GameObject obj in objects)
        {
            Enum objectType = obj.GetComponent<Box>().type;
        ...
        foreach (KeyValuePair<Enum, int> pair in typeCounts)
        {
            if (pair.Value >= 3)
            {
